...
答：一般用@Controller 注解,表示是表现层,不能用用别的注解代替。
...
答：在返回值前面加"forward:"就可以让结果转发,譬如"forward:user.do?name=method4" 在 返回值前面加"redirect:"就可以让返回值重定向,譬如"redirect:http://www.baidu.com"
